...
const onsWithRowHierarchy = sheet => {
  ...
  console.log('accesing data example: ', sheet.D12.v);

  const restOfRowEmpty = (startCell, stopAt) => {
  ...
  headersStart.forEach (header => {
    while (!restOfRowEmpty(header, 50)) {
      console.log(sheet[header].v);
      // console.log(header);
      header = incX (header);
      while (!sheet[header] && !restOfRowEmpty(header, 50))
        header = incX (header);
    }
    console.log('OK then');
  })
